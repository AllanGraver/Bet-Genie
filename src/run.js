 ...
 * API-Football er et valgfrit supplement til fixtures, catch-up og H2H.
 * config/leagues.json er masterfil for ligaer, API-ID'er, TheRundown-ID'er
 * og fallback-links. Odds tilføjes efterfølgende af src/enrich-odds.js.
 */
...
const leaguesFile = path.join(root, 'config', 'leagues.json');
...
if (!Array.isArray(leagueConfiguration)) {
  throw new Error('config/leagues.json skal indeholde et JSON-array.');
}
...
const missingLeagueSlugs = leagueConfiguration.filter(
  league => !String(league?.slug || '').trim()
);
...
const configuredLeagueSlugs = leagueConfiguration.map(
  league => String(league.slug).trim()
);

const duplicateLeagueSlugs = configuredLeagueSlugs.filter(
...
for (const league of leagueConfiguration) {
  league.slug = String(league.slug).trim();

  if (
    league.filePatterns !== undefined &&
    !Array.isArray(league.filePatterns)
  ) {
    throw new Error(`${league.slug}: filePatterns skal være et array.`);
  }
  ...
  if (
    ...
    throw new Error(
      `${league.slug}: rundownSportId skal være et positivt tal eller null.`
    );
  }
  ...
  if (
    ...
    const fallbackUrl = normalizeHttpsUrl(league.fallbackOddsUrl);

    if (!fallbackUrl) {
      throw new Error(
        `${league.slug}: fallbackOddsUrl skal være en gyldig HTTPS-URL.`
      );
    }

    league.fallbackOddsUrl = fallbackUrl;
  }
}

const leagueConfigurationBySlug = new Map(
  leagueConfiguration.map(league => [league.slug, league])
);
...
function findLeagueConfiguration(leagueOrMatch) {
  ...
  const slug = String(
    leagueOrMatch.slug || leagueOrMatch.leagueSlug || ''
  ).trim();

  if (slug && leagueConfigurationBySlug.has(slug)) {
  ...
  for (const configuredLeague of leagueConfiguration) {
    const configuredCandidates = uniqueNormalizedValues([
      configuredLeague.slug,
      configuredLeague.displayName,
      ...(Array.isArray(configuredLeague.filePatterns)
...
function getFallbackOddsUrl(leagueOrMatch) {
  const league = findLeagueConfiguration(leagueOrMatch);
  const configuredUrl = normalizeHttpsUrl(league?.fallbackOddsUrl);

  return configuredUrl || normalizeHttpsUrl(defaultFootballOddsUrl);
}
...
function getRundownSportId(leagueOrMatch) {
  ...
  const sportId = Number(league.rundownSportId);

  return Number.isFinite(sportId) && sportId > 0
    ? sportId
    : null;
}

function getLeagueOddsConfiguration(leagueOrMatch) {
  const rundownSportId = getRundownSportId(leagueOrMatch);
  const rundownSupported = rundownSportId !== null;

  return {
    source: rundownSupported ? 'therundown' : 'external-link',
    provider: rundownProviderName,
    bookmaker: rundownBookmakerName,
    bookmakerId: rundownBookmakerId,
    rundownSupported,
    rundownSportId,
    fallbackUrl: getFallbackOddsUrl(leagueOrMatch),
    fallbackLabel: 'Find odds hos Unibet'
  };
}

function enrichMatchWithOddsConfiguration(match) {
  const oddsConfiguration = getLeagueOddsConfiguration(match);

  return {
    ...match,
    oddsConfiguration,
    oddsStatus: oddsConfiguration.rundownSupported
      ? 'pending'
      : 'external-link',
    oddsSource: oddsConfiguration.rundownSupported
      ? rundownProviderName
      : null,
    bookmaker: rundownBookmakerName,
    fallbackOddsUrl: oddsConfiguration.fallbackUrl,
    fallbackOddsLabel: oddsConfiguration.fallbackLabel
  };
}
...
const analysisDate =
  environmentAnalysisDate || getCurrentDateInTimezone(timezone);
...
const analysisDateSource = environmentAnalysisDate
  ? 'environment'
  : 'system-date';
...
console.log(
  'BetScope analysedato:',
  {
    environmentAnalysisDate: environmentAnalysisDate || null,
    resolvedAnalysisDate: analysisDate,
    timezone,
    source: analysisDateSource
  }
);
...
function matchKey(match) {
  ...
  return [
    ...
    normalizeText(match.leagueSlug || match.leagueName)
  ].join('|');
}
  ...
  for (const collection of collections) {
    ...
    for (const match of collection) {
      ...
      if (incomingFinished && !existingFinished) {
        matchMap.set(key, { ...existing, ...match });
        continue;
      }
      ...
      matchMap.set(key, {
        ...
        leagueSlug: existing.leagueSlug || match.leagueSlug || null,
        leagueName: existing.leagueName || match.leagueName || null
      });
    }
  }
...
function periodFixturesFromCsv(matches, fromDate, toDate) {
...
async function readH2HCache(file, maximumAgeInDays) {
  try {
    const cached = JSON.parse(await fs.readFile(file, 'utf8'));

    if (!cached.fetchedAt || !Array.isArray(cached.matches)) {
      return null;
    }

    const fetchedAtTime = new Date(cached.fetchedAt).getTime();

    if (!Number.isFinite(fetchedAtTime)) {
      return null;
    }

    const ageInDays = (Date.now() - fetchedAtTime) / 86400000;

    return ageInDays <= maximumAgeInDays
      ? cached
      : null;
  } catch {
    return null;
...
function h2hCandidateKeys(
  ...
  for (const matches of matchesByDate.values()) {
    ...
    selected.push(...matches.slice(0, maximumPerDay));
  }
...
function validateLeagueForApiFootball(league) {
  if (!league) {
    return false;
  }

  const leagueId = Number(league.apiLeagueId);
...
const foundLeagueFiles = Array.isArray(discovery.found)
  ? discovery.found
  : [];

const unknownCsvFiles = Array.isArray(discovery.unknown)
  ? discovery.unknown
  : [];
...
for (const leagueFile of foundLeagueFiles) {
  const league =
    findLeagueConfiguration(leagueFile.league) ||
    leagueFile.league;
  ...
  if (!validateLeagueForApiFootball(league)) {
    warnings.push(
  ...
  if (!apiFootballLeagueMap.has(apiLeagueId)) {
    apiFootballLeagueMap.set(apiLeagueId, league);
  }
}

let allHistory = mergeMatches(
  foundLeagueFiles.flatMap(file =>
    Array.isArray(file.matches) ? file.matches : []
  )
);
...
if (apiFootball && apiFootballLeagueMap.size > 0) {
  ...
  console.log('API-Football fixtureperiode:', {
    ...
    to: analysisEndDate,
    timezone,
    activeLeagues: apiFootballLeagueMap.size
  });

  for (const league of apiFootballLeagueMap.values()) {
...
const preAnalyses = periodFixtures
  .map(fixture =>
    preAnalyse(
      fixture,
      allHistory.filter(
        match => match.leagueSlug === fixture.leagueSlug
      ),
      settings
    )
  )
...
for (const candidate of preAnalyses) {
  ...
  const shouldRequestH2H = (
    ...
    h2hMatches.length < Number(settings.h2hMinimumMatches || 5) &&
    candidate.homeId &&
    candidate.awayId
  );
      ...
      try {
        ...
        h2hMatches = response.map(item => {
          const responseLeague =
            apiFootballLeagueMap.get(Number(item.league?.id)) ||
            {
              slug: candidate.leagueSlug,
              displayName:
                item.league?.name || candidate.leagueName,
              apiLeagueId: item.league?.id,
              season: item.league?.season
            };

          return apiFixtureToMatch(item, responseLeague);
        });
...
/* ------------------------------------------------------------
 * ODDSKONFIGURATION, SORTERING OG OPDELING
 * ------------------------------------------------------------ */

const enrichedFinalResults = finalResults.map(
  enrichMatchWithOddsConfiguration
);

enrichedFinalResults.sort(
  (matchA, matchB) =>
    Number(matchB.passed) - Number(matchA.passed) ||
    Number(matchB.score || 0) - Number(matchA.score || 0)
);

const approvedResults = enrichedFinalResults.filter(
  match => match.passed
);

const nearMisses = enrichedFinalResults.filter(
  match => !match.passed
);
...
for (const leagueFile of foundLeagueFiles) {
  ...
  const key =
    configuredLeague.slug ||
    normalizeText(configuredLeague.displayName);
  ...
  if (!leagueSummaryMap.has(key)) {
    const oddsConfiguration =
      getLeagueOddsConfiguration(configuredLeague);

    const apiFootballConfigured =
      validateLeagueForApiFootball(configuredLeague);

    leagueSummaryMap.set(key, {
      slug: configuredLeague.slug || key,
      name:
        configuredLeague.displayName ||
        configuredLeague.slug ||
        'Ukendt liga',
      apiFootball: {
        ...
        leagueId: apiFootballConfigured
          ? Number(configuredLeague.apiLeagueId)
          : null,
        season: apiFootballConfigured
          ? Number(configuredLeague.season)
          : null
      },
      odds: {
        provider: oddsConfiguration.provider,
        bookmaker: oddsConfiguration.bookmaker,
        bookmakerId: oddsConfiguration.bookmakerId,
        rundownSupported: oddsConfiguration.rundownSupported,
        rundownSportId: oddsConfiguration.rundownSportId,
        status: oddsConfiguration.rundownSupported
          ? 'pending'
          : 'external-link',
        fallbackUrl: oddsConfiguration.fallbackUrl,
        fallbackLabel: oddsConfiguration.fallbackLabel
      },
      csvRows: 0,
      csvFiles: 0,
      periodMatches: 0
  ...
  summary.csvRows += Number(
    leagueFile.rowCount ||
      (Array.isArray(leagueFile.matches)
...
for (const fixture of periodFixtures) {
  const configuredLeague = findLeagueConfiguration(fixture);
...
const foundLeagues = Array.from(
  ...
  (leagueA, leagueB) =>
    leagueA.name.localeCompare(leagueB.name, 'da')
);

sourceStatus.odds.configuredLeagues = foundLeagues.length;
sourceStatus.odds.supportedLeagues = foundLeagues.filter(
  league => league.odds?.rundownSupported
).length;
sourceStatus.odds.fallbackLeagues = foundLeagues.filter(
  league => !league.odds?.rundownSupported
).length;
...
const matchesByDate = Array.from(
  ...
  .sort()
  .map(date => {
    const dateMatches = enrichedFinalResults.filter(
...
const output = {
  ...
  oddsConfiguration: {
    ...
    defaultFallbackUrl: normalizeHttpsUrl(
  ...
  requestUsage: {
    ...
    hardLimit: Number(settings.dailyHardLimit || 85)
  },
